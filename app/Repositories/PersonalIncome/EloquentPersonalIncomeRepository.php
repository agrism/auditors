...
class EloquentPersonalIncomeRepository implements PersonalIncomeRepository {
    ...
    public function getPersonalIncomes(array $params)
    {
        ...
        $invoice = PersonalIncome::select(\DB::raw('personal_incomes.*, partners.name as partnername,
        personal_income_types as personal_income_type' ) )
            ->leftJoin('partners', 'personal_incomes.partner_id', '=', 'partners.id')
            ->leftJoin('personal_income_types', 'personal_incomes.personal_income_type_id', '=', 'personal_income_type.id')
...
//		dd($params);
        if (isset($params['filter']))
        {
            ...
            if (isset($filter['personal_income_type_id']) && $filter['personal_income_type_id'] != '')
            {
                $invoice = $invoice->where('personal_income_type_id', $filter['personal_income_type_id']);
            }
        }
//		dd($filter);
