...
//        dd($request->all());
        ...
        if ($request->get('submitValue') && $request->get('submitValue') == 'Generate NPI xml') {

            $fileName =  $this->createXML();
        }

        return view('admin.npi.create', ['dataArray' => $this->dataArray], compact('fileName'));
    }


    private function getEndOfMonthDateBeforeReport(Carbon $date, Carbon $reportDate, Carbon $lastDate)
    {
        ...
        $this->getEndOfMonthDateBeforeReport((clone $date)->addMinute(), $reportDate, $lastDate);
    }
...
//        dump($this->dates);
        ...
        forEach ($this->dates as $key => $date) {
            ...
            $this->dataArray[] = [
                ...
                'amount' => Round($amount / $allPeriodDays * $date->diffInDays($prevDate), 2)
            ];
        ...
        forEach ($this->dataArray as $key => $val) {
            ...
            $this->dataArray[$key]['comment'] = $request->get('description') . ' (norakst.' . ($key + 1) . ' no ' . count($this->dataArray).') Rekins:'.$request->get('invoice_no');
            $this->dataArray[$key]['number'] = $request->get('invoice_no') . ' norak. ' . ($key + 1) . ' no ' . count($this->dataArray);
