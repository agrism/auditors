...
function sort_employees_by($column, $body, $params = [])
{
	...
	if (is_int($key)) {
		$currentColumnDirection = isset($params['sort'][$key]['direction'])
			? $params['sort'][$key]['direction'] : false;
	}
	...
	array_unshift(
		$params['sort'],
		['orderBy' => $column, 'direction' => $newColumnDirection]
	); // push first current column

	$params = removeColumnDuplicates($params);


	return link_to_route1(
		'client.invoices.index',
		$body.'&nbsp;<div class="fa '.$faSort.'"></div>', $params
	);
}
